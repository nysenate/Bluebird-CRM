...
class Meta {
  ...
  public static function formatFieldSpec(array $column, array $expr): array {
    ...
    if ($expr['expr']->getType() === 'SqlField') {
      // Pseudo-fields (e.g. SearchSegment fields) have no entity; skip entity-specific metadata for them
      if (!empty($field['entity'])) {
        // An entity id counts as a FK
        if (!($field['fk_entity'] ?? NULL) && $field['name'] === CoreUtil::getIdFieldName($field['entity'])) {
