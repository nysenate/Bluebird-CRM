...
cLog(0,'111');
	...
	while ($ctRow) {
		...
		if ($omis_ext) {
			...
			if ($otherSuffix!='null') $nonOmis.='Other Suffix: '.$otherSuffix;
...
//echo "\n";
		...
		//if we don't have the relationship,write it out
                ...
                $params['external_identifier'] = $sourceDesc.$importID.'-H';
                $params['first_name'] = 'NULL';
                $params['middle_name'] = 'NULL';
                $params['last_name'] = 'NULL';
...
print_r("\ncreating household $contactID: ".$aRel['household_name']);
