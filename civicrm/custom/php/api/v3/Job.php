...
function civicrm_api3_job_fetch_bounces($params) {
  $lock = Civi\Core\Container::singleton()->get('lockManager')->acquire('worker.mailing.EmailProcessor');//NYSS 8629
  if (!$lock->isAcquired()) {
...
function civicrm_api3_job_process_membership($params) {
  $lock = Civi\Core\Container::singleton()->get('lockManager')->acquire('worker.member.UpdateMembership');//NYSS 8629
  if (!$lock->isAcquired()) {
...
function civicrm_api3_job_group_rebuild( $params ) {
  $lock = Civi\Core\Container::singleton()->get('lockManager')->acquire('worker.core.GroupRebuild');//NYSS 8629
  if (!$lock->isAcquired()) {
    return civicrm_api3_create_error('Could not acquire lock, another EmailProcessor process is running');
  }
