...
class CRM_Core_BAO_EntityTag extends CRM_Core_DAO_EntityTag 
{
     ...
     *
     * @return  array   $entityIds    array of entityIds ids
     * @access public
     */
    function getEntitiesByTag($tag)
    {
        // NYSS - getEntitiesByTag should return the entity, not the contact.
        // If its meant to match on the contact_id, it should be named getContactsByTag
        $entityIds = array();
    ...
    static function getChildEntityTagDetails( $parentId, $entityId, $entityTable = 'civicrm_contact' ) {
		...
		$entityTagDetails = array( );
        $query = "SELECT ct.id as tag_id, name, description FROM civicrm_tag ct
                    INNER JOIN civicrm_entity_tag et ON ( et.entity_id = {$entityId} AND
                     et.entity_table = '{$entityTable}' AND  et.tag_id = ct.id)
                  WHERE ct.parent_id = {$parentId}";
        $dao = CRM_Core_DAO::executeQuery( $query );
        while( $dao->fetch( ) ) {
            $entityTagDetails[$dao->tag_id] = array( 'id'   	   => $dao->tag_id,
                                               		 'name' 	   => $dao->name,
