...
class CRM_Contact_Form_Search_Builder extends CRM_Contact_Form_Search
{
    ...
    static function formRule( $values ) {
        ...
        $errorMsg = array ();
        foreach ($fld as $k => $v) {
          $v[2] = self::checkArrayKeyEmpty($v[2]);//NYSS 2037

            if ( !$v[1] ) {
                $errorMsg["operator[$v[3]][$v[4]]"] = ts("Please enter the operator.");
            } else {
                if ( in_array( $v[1], array( 'IS NULL', 'IS NOT NULL' ) ) && !empty($v[2]) ) {//NYSS 2037
                    $errorMsg["value[$v[3]][$v[4]]"] = ts('Please clear your value if you want to use %1 operator.', array( 1 => $v[1] ));  
                } else if ( ( $v[0] == 'group' || $v[0] == 'tag' ) && !empty($v[2]) ) {//NYSS 2037
    ...
    public function postProcess( ) {
        ...
        //NYSS check if value is empty array
