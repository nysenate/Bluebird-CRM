...
trait CRM_Contact_Form_Task_EmailTrait {
  ...
  public function buildQuickForm() {
    ...
    if (!empty($this->_allContactIds) && $setDefaults) {
      ...
      //NYSS 5389/7361/5873 attempt to retrieve primary email
      $allContactDetails = civicrm_api3('Contact', 'get', [
        'id' => ['IN' => $this->_allContactIds],
        'return' => ['sort_name', 'email', 'do_not_email', 'is_deceased', 'on_hold', 'display_name', 'preferred_mail_format', 'is_primary'],
        'options' => ['limit' => 0, 'sort' => 'is_primary DESC'],
      ])['values'];
      ...
      if (empty($toArray)) {
        ...
        if ($this->_single) {
          ...
          if ($this->_searchContext) {
            $urlContext = $this->_searchContext;
          }

          $urlRedirect = CRM_Utils_System::url('civicrm/contact/search/'.$urlContext, $urlParams);
          if (isset($this->_activityHolderIds)) {
            $urlRedirect = CRM_Utils_System::url('civicrm/activity/search/'.$urlContext, $urlParams);
          }
        }
        CRM_Core_Error::statusBounce(ts('Selected contact(s) do not have a valid email address, or communication preferences specify DO NOT EMAIL, or they are deceased or Primary email address is On Hold.'), $urlRedirect);
      }
    }
