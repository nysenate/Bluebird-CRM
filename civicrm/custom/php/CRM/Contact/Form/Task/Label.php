...
class CRM_Contact_Form_Task_Label extends CRM_Contact_Form_Task 
{
    ...
    function buildQuickForm()
    {
        ...
        $this->add( 'checkbox', 'merge_same_household', ts( 'Merge labels for contacts with the same household' ), null );
    ...
    public function postProcess ( )
    {
        ...
        if ( isset( $fv['merge_same_address'] ) ) {
         ...
         //start of code changes done by Web Access 
        if ( isset( $fv['merge_same_household'] ) ) {
            require_once 'api/v2/Relationship.php';
            foreach( $rows as $cId => $details ) {
                $relParams = array( 'contact_id' => $cId );
                $rel = civicrm_contact_relationship_get( $relParams );
                if ( !$rel['is_error'] && !empty($rel['result'])) {
                    foreach( $rel['result'] as $relContactId => $relDetails ) {
                        if ( $relDetails['civicrm_relationship_type_id'] == 7 ) {
                            $rows[$relDetails['cid']]= $relDetails;
                            unset ( $rows[$cId] ); 
                            break;
                        }
                    }
         ...
         // end of code changes done by Web Access 
        ...
        foreach ( $uniqueAddress as $address => $data) {	// copy data back to $rows
            ...
            foreach ( $data['names'] as $last_name => $first_names ) {	// one last name list per row  
                ...
                if ($count) {
                    ...
                    $rows[$data['ID']]['addressee_display'] =  $family;
                    // end of code changes done by Web Access 
                }
                $count++;
            }
