    ...
    if ($this->_parseStreetAddress) {
      if (array_key_exists('address', $formatted) && is_array($formatted['address'])) {
        foreach ($formatted['address'] as $instance => & $address) {
          ...
          $streetAddress = CRM_Utils_Array::value( 'street_address', $address );
          $streetName = CRM_Utils_Array::value( 'street_name', $address );
          $streetNumber = CRM_Utils_Array::value( 'street_number', $address );
          $streetUnit = CRM_Utils_Array::value( 'street_unit', $address );
          
          //if street address set and street name/number empty, parse fields
          if ( !empty( $streetAddress ) && ( empty( $streetName ) || empty( $streetNumber ) ) ) {
            // parse address field.
...
//if street address not set, and name or number is, concatenate values
          }
          elseif ( !empty( $streetName ) || !empty( $streetNumber ) ) {
            
          $parsedFields['street_address'] = trim($streetNumber.' '.$streetName.' '.$streetUnit);
          //if both street address and parsed values are imported, conduct no parse-related activities  
          }
          else {
            continue;
          }
