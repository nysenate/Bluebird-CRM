...
class CRM_Contact_BAO_Individual extends CRM_Contact_DAO_Contact
{
    ...
    static function format( &$params, &$contact )
    {
		...
		//NYSS moved earlier in code so email is defined before display_name is set
        ...
        if ( $lastName || $firstName || $middleName || $email ) { //NYSS 2744
            $tokens = array( );
			...
			if ( empty($displayName) ) $displayName = $email; //NYSS 2744
        }
