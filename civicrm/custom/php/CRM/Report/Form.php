...
class CRM_Report_Form extends CRM_Core_Form {
  ...
  public function buildInstanceAndButtons() {
    ...
    //NYSS 2489
    $label = $this->_id ? ts( 'Create PDF' ) : ts( 'Create PDF' );
    $this->addElement('submit', $this->_pdfButtonName, $label);

    //NYSS 2489
    $label = $this->_id ? ts('Export to CSV') : ts( 'Create CSV' );
...
WHERE cg.extends IN ('" . implode("','", $this->_customGroupExtends) . "') AND
  ...
  public function endPostProcess(&$rows = NULL) {
    ...
    if ($this->_outputMode == 'print' ||
      ...
      $content = str_replace( '</head>', '<style type="text/css">@import url('.$config->userFrameworkBaseURL.'sites/default/themes/Bluebird/css/printCivicrm.css);</style></head>', $content );
