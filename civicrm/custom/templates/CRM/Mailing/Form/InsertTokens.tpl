...
function tokenReplText(element) {
  ...
  //NYSS 6593 doesn't like all that much
...
function showToken(element, id ) {
  //NYSS creates a data item called getPosition that's attached to the body
  var cjSetInput = cj("#"+element.toLowerCase()).parents('td').children('input').focus();
  //if the array has a value, get the selection of it.
  if(cjSetInput.length > 0) {
