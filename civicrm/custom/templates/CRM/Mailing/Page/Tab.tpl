...
<div class="crm-mailing-selector">
  <table class="contact-mailing-selector crm-ajax-table">
    <thead>
      {*NYSS 6698*}
      <tr>
        <th data-data="subject" class="crm-mailing-contact-subject">{ts}Mailing Name{/ts}</th>{*NYSS 6895*}
        <th data-data="creator_name" class="crm-mailing-contact_created">{ts}Added By{/ts}</th>
        <th data-data="recipients" data-orderable="false" class="crm-contact-activity_contact">{ts}Recipients{/ts}</th>{*NYSS 6890*}
        <th data-data="start_date" class="crm-mailing-contact-date">{ts}Date Sent{/ts}</th>{*NYSS 6892*}
