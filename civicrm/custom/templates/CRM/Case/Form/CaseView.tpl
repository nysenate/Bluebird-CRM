      ...
      <tr class="crm-case-caseview-client">
        <td colspan="5" class="label">
          ...
          {foreach from=$caseRoles.client item=client name=clients}
            {*NYSS 14983*}
            <a href="{crmURL p='civicrm/contact/view' q="action=view&reset=1&cid=`$client.contact_id`"}" title="{ts}View contact record{/ts}">{$client.display_name}</a><a class="crm-popup crm-hover-button" href="{crmURL p='civicrm/contact/view/case/deleteClient' q="action=delete&reset=1&cid=`$client.contact_id`&id=`$caseId`&rcid=`$contactID`"}" title="{ts}Remove Client{/ts}"><i class="crm-i fa-times" aria-hidden="true"></i></a>{if not $smarty.foreach.clients.last}, &nbsp; {/if}
          {/foreach}
