    ...
    <tr>
      <td>
        <div class="crm-submit-buttons"> 
          ...
          {*NYSS 6381*}
          {if $addressId}
            &nbsp;<a class="button delete-button" href="#" style="display:block;" aid={$addressId}><span style="padding:5px 5px 5px 0"><div class="icon delete-icon"></div> {ts}Delete{/ts}</span></a>
...
cj( function() {
  ...
  //NYSS 6381 (heavily modified for backporting; revert in 4.3.x)
  cj('.crm-inline-edit-form.crm-edit-address-form .delete-button').click(function() {
    var block = cj(this).closest('.crm-address-block');
    var aid = cj(this).attr('aid');
    if ( confirm('Are you sure you want to delete this address?') ) {
      cj().crmAPI('address', 'delete', {id: aid}, {
        success:
        function(data) {
          cj(block).remove();
        }
