(function(angular, $, _) {
  ...
  angular.module('crmMosaico').controller('NYSSMosaicoReview', function($scope) {
    var mids = [];
    var gids = [];
    var groupNames = [];
    var mailings = [];
      ...
      if (-1 == mailings.indexOf(mailing.id)) {
        ...
        if (!_.isEmpty(gids)) {
          CRM.api3('Group', 'get', {'id': {"IN": gids}}).then(function(result) {
            ...
            CRM.crmMosaico.groupNames = groupNames;
            //$scope.$parent.crmMosaicoConst.groupNames = groupNames;
          });
        }
      }
    }
      ...
      if (-1 == civimailings.indexOf(mailing.id)) {
        ...
        if (!_.isEmpty(mids)) {
          CRM.api3('Mailing', 'get', {'id': {"IN": mids}}).then(function(result) {
            ...
            CRM.crmMosaico.civiMails = civimails;
            //$scope.$parent.crmMosaicoConst.civiMails = civimails;
          });
        }
      }
    }
    ...
    $scope.getIncludesAsString = function(mailing) {
      var first = true;
      var names = '';
      if (_.isEmpty(CRM.crmMosaico.groupNames)) {
        getGroupNames(mailing);
      }
      if (_.isEmpty(CRM.crmMosaico.civiMails)) {
        getCiviMails(mailing);
      }
      _.each(mailing.recipients.groups.include, function(id) {
        var group = _.where(CRM.crmMosaico.groupNames, {id: parseInt(id)});
        if (group.length) {
          if (!first) {
            names = names + ', ';
          }
      ...
      _.each(mailing.recipients.mailings.include, function(id) {
        var oldMailing = _.where(CRM.crmMosaico.civiMails, {id: parseInt(id)});
    ...
    $scope.getExcludesAsString = function(mailing) {
      ...
      //console.log('getExcludesAsString names: ', names);
      return names;
    };
  });
})(angular, CRM.$, CRM._);
