...
class CRM_migrateContactsImport {
  ...
  function importContacts(&$exportData, &$stats, $bbAdmin) {
    ...
    //bbscript_log("info", 'extInt', $extInt);exit();
  }//importContacts
  ...
  function importActivities($exportData, $bbAdmin) {
    ...
    bbscript_log("info", 'importData $actCount', $actCount);
  }//importActivities
  ...
  function importCases($exportData, $bbAdmin) {
      ...
      foreach ( $cases as $case ) {
        ...
        foreach ( $activities as $oldID => $activity ) {
          ...
          if ( empty($activity['parent_id']) ) {
            //bbscript_log("trace", 'importCases $activity[parent_id]', $activity['parent_id']);
            continue;
          }
          else {
            $actCount++;
    ...
    bbscript_log("info", 'importCases $actCount', $actCount);
  }//importCases
  ...
  function importTags($exportData) {
    ...
    //bbscript_log("trace", '_importTags $extInt', $extInt);
    foreach ( $exportData['tags']['entities'] as $extID => $extTags ) {
      ...
      if ( empty($params['contact_id']) ) {
        bbscript_log("trace", "_importTags extID (no contact_id): $extID");
      }
      else {
  ...
  function importHouseholdRels(&$exportData) {
    ...
    bbscript_log("info", "importing household relationships...");
