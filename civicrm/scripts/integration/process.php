  ...
  function run()
  {
    ...
    while ($row->fetch()) {
        ...
        if ($row->target_shortname != $row->user_shortname &&
            ...
            $this->dryrun(function() use ($this, $row) {
        ...
        */
        ...
        try {
            $this->dryrun(function() use ($cid, $event_data) {
        ...
        try {
            $result = $this->dryrun(function() use ($web_event, $cid) {
            ...
            if ($this->dry) {
                ...
                $this->dryrun(function() use ($cid, $activity_type, $event_data, $activity_details, $activity_data) {
      ...
      switch ($row->event_type) {
        case WebsiteEvent::EVENT_TYPE_BILL:
            try {
                ...
                $result = $this->dryrun(function() use ($bill_event) {
      ...
      if ($result['is_error'] || $result == FALSE) {
        ...
        if ($this->>optlist['archive']) {
          bbscript_log(LL::DEBUG, 'Archiving matched/created record to '. $web_event->getArchiveTableName().' and archive_error table');
            $this->dryrun(function() use ($row, $web_event) {
        ...
        if ($this->>optlist['archive']) {
          bbscript_log(LL::DEBUG, 'Archiving matched/created record to '. $web_event->getArchiveTableName() .' table');
            $this->dryrun(function() use ($web_event, $row) {
    ...
    public function archiveError(CRM_NYSS_BAO_Integration_WebsiteEventInterface $web_event, $row, string $reason = '') : void
    {
        if (! $this->optlist['archive']) return; // archive flag not set... don't do it

        try {
            bbscript_log(LL::DEBUG, "Archiving record to " . $web_event->getArchiveTableName() . " and archive_error table: " . $reason);
            $this->dryrun(function() use ($web_event, $row) {
                CRM_NYSS_BAO_Integration_Website::archiveRecord($this->intDB, $web_event, $row, $web_event->getEventInfo(), false);
            }, "CRM_NYSS_BAO_Integration_Website::archiveRecord");
        } catch (Exception $e) {
            bbscript_log(LL::WARN, "An unexpected error occurred while archiving a failed record.");
    ...
    public function archiveSuccess(CRM_NYSS_BAO_Integration_WebsiteEventInterface $web_event, $row, string $message = ''): void
    {
        ...
        try {
            bbscript_log(LL::DEBUG, 'Archiving record to archive and '. $web_event->getArchiveTableName() .' table: ' . $message);
            $this->dryrun(function() use ($web_event, $row) {
                CRM_NYSS_BAO_Integration_Website::archiveRecord($this->intDB, $web_event, $row, $web_event->getEventInfo());
            }, "CRM_NYSS_BAO_Integration_Website::archiveRecord");
        } catch(Exception $e) {
            bbscript_log(LL::WARN, "An unexpected error occurred while archiving a record.");
            CRM_NYSS_Errorhandler_BAO::notifySlack('Error Archiving Website Event Record: ' . $e->getMessage() ."\nDATA:\n" . var_export($row,true));
            CRM_NYSS_Errorhandler_BAO::notifyEmail('Error Archiving Website Event Record: ' . $e->getMessage() ."\nDATA:\n" . var_export($row,true), 'Error Archiving Website Event');
            // allow processing to continue
        }
    }
