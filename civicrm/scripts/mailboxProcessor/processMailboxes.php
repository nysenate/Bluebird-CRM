...
if (!isset($aIncomingIMAPAccount[$site])) {
  die("no mailboxes to process...");
}

foreach (/* USE NEW BLUEBIRD CONFIG FILE FOR ACCOUNTS/LOGINS/PASSWORDS */) {
  $instanceName = ""; /* column 1 */
  $imapUser = ""; /* column 6 */
  $imapPass = ""; /* column 7 */
  cLog(0, 'info', "polling {$instanceName}...");
  $connection = imap_open("{".IMAP_SERVER.IMAP_OPTS."}", $imapUser, $imapPass);
  ...
  if (IMAP_MOVE_MAIL_TO_ARCHIVE == true) {
    @imap_createmailbox($connection, imap_utf7_encode("{".IMAP_SERVER."}".IMAP_MAILBOX.".".IMAP_ARCHIVEBOX));
  }
  ...
  for ($i = 1; $i <= $count; $i++) {
    ...
    if ($structure->type == 1) {
      $email->body = imap_fetchbody($connection, $i, "1");
    }
    else {
      $email->body = imap_body($connection, $i);
    }
