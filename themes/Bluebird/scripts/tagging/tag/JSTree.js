...
(function() {
  var Action, ActivityLog, Autocomplete, Buttons, Entity, Instance, Label, Node, OpenLeg, Resize, Settings, Tree, View, jstree, setProp, _descWidths, _getTrees, _manipTags, _openTags, _parseAutocomplete, _parseTree, _tree, _treeUtils, _treeVisibility, _utils,
    __slice = [].slice,
    _this = this,
  ...
  jstree = {
    init: function(settings, view) {
      var dataSettings, instance, pageElements, request,
        _this = this;
  ...
  Instance = (function() {
    ...
    Instance.prototype.removeFromAC = function(node) {
      var i, v, _i, _len, _ref, _results;
      _ref = this._autocomplete;
      _results = [];
      for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
        v = _ref[i];
        if (parseInt(node.id) === parseInt(v.id)) {
          console.log("removeAC " + i);
          _results.push(this._autocomplete.splice(i, 1));
        } else {
  ...
  _utils = {
    ...
    textWrap: function(text, length) {
      var a, currentEnd, currentLastSpace, killFutureSegs, lastEnd, nextSpace, numberOfSegs, retObj, rx, seg, shouldRet, wordPiece, _i, _j, _len, _ref;
  ...
  Entity = (function() {
    ...
    Entity.prototype.entity_id = 0;

    Entity.prototype._get = {
    ...
    function Entity(entity_id, cb) {
      var a, i, request, _i, _len,
        _this = this;
      this.entity_id = entity_id;
      a = ["_get", "_create", "_del"];
      for (_i = 0, _len = a.length; _i < _len; _i++) {
        i = a[_i];
        this[i].data.entity_id = this.entity_id;
      }
      request = cj.when(cj.ajax(this._get));
      request.done(function(data) {
        _this.tags = data["message"];
        return cb.call(_this, _this.tags);
      });
      return this;
    }

    Entity.prototype.addTag = function(tagId) {
      this._create.data.tag_id = tagId;
      return cj.when(cj.ajax(this._create));
    };

    Entity.prototype.removeTag = function(tagId) {
      var index;
      this._del.data.tag_id = tagId;
      index = this.tags.indexOf(tagId);
      if (index > -1) {
        this.tags.splice(index, 1);
      }
      return cj.when(cj.ajax(this._del));
    };
    ...
    View.prototype.applyTagged = function() {
      ...
      return this.instance.getEntity(this.entity_id, function(tags) {
        ...
        if (_this.entityList.length > 0) {
          ...
          return _this.addPositionReminderText(_this.cj_selectors.tagBox.find(".top-292"));
        }
      });
    };
