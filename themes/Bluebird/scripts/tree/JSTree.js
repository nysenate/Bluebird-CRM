...
(function() {
  var BBTagLabel, View, getTrees, instance, parseTree, tree, treeBehavior, _ref, _ref1, _treeData, _treeVisibility, _viewSettings,
    __slice = [].slice,
  ...
  View = (function() {
    ...
    View.prototype.writeTreeFromSource = function() {
      ...
      for (key in _treeVisibility) {
        val = _treeVisibility[key];
        _treeVisibility[key] = "top-" + this.displaySettings.defaultTree;
      }
  ...
  treeBehavior = {
        ...
        return searchmonger.exec(event, function(terms) {
          ...
          if (cjac.val().length < 3) {
            if (_treeVisibility.currentTree === "search") {
              return _this.showTags(_treeVisibility.previousTree);
            }
          }
        });
      });
    ...
    buildSearchList: function(tagList, term) {
      var cjCloneChildren, cjCloneTag, key, tag;
      ...
      if (tagList !== null) {
        for (key in tagList) {
          ...
          console.log(this.cjTagBox);
          console.log(this.cjTagBox.find("#tagDropdown_" + tag.id));
          cjCloneChildren = this.cjTagBox.find("#tagDropdown_" + tag.id + "]");
          cjCloneChildren.clone().appendTo(this.cjSearchBox);
          this.enableDropdowns(".search dt[data-tagid=" + tag.id + "]");
        }
      } else {
    ...
    showTags: function(currentTree, noPrev) {
      if (currentTree !== _treeVisibility.currentTree) {
        ...
        return this.cjTagBox.find("." + currentTree).toggle();
      }
    },
