...
// Add Zen Tabs styles
if (theme_get_setting('BluebirdPublic_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'BluebirdPublic') .'/css/tabs.css');
}

function BluebirdPublic_preprocess_html(&$vars) {
  global $user, $language;

  // Add role name classes (to allow css based show for admin/hidden from user)
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . BluebirdPublic_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = BluebirdPublic_id_safe('page-'. $path);
    $vars['classes_array'][] = BluebirdPublic_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('BluebirdPublic_ie_enabled')) {
    $BluebirdPublic_ie_enabled_versions = theme_get_setting('BluebirdPublic_ie_enabled_versions');
    if (in_array('ie8', $BluebirdPublic_ie_enabled_versions, TRUE)) {
    ...
    if (in_array('ie9', $BluebirdPublic_ie_enabled_versions, TRUE)) {
      drupal_add_css(path_to_theme() . '/css/ie9.css', array('group' => CSS_THEME, 'browsers' => array('IE' => 'IE 9', '!IE' => FALSE), 'preprocess' => FALSE));
    }
    if (in_array('ie10', $BluebirdPublic_ie_enabled_versions, TRUE)) {
...
function BluebirdPublic_preprocess_page(&$vars, $hook) {
...
function BluebirdPublic_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from BluebirdPublic_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function BluebirdPublic_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function BluebirdPublic_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('BluebirdPublic_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('BluebirdPublic_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('BluebirdPublic_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('BluebirdPublic_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('BluebirdPublic_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 *
 */
function BluebirdPublic_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = BluebirdPublic_id_safe($element['#title']);
 ...
 */
function BluebirdPublic_preprocess_menu_local_task(&$variables) {
 ...
 */
function BluebirdPublic_menu_local_tasks(&$variables) {
  $output = '';
